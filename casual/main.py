...
class firework(object):
    def __init__(self, color, speed, width, height):
        ...
        for i in range(self.nParticle):
            ...
            x, y = self.ellipsePara[0] * math.cos(
                theta * i + t), self.ellipsePara[1] * math.sin(
                    theta * i + t)  #椭圆参数方程
            xx, yy = x * math.cos(self.rotTheta) - y * math.sin(
                self.rotTheta), y * math.cos(self.rotTheta) + x * math.sin(
                    self.rotTheta)  #平面旋转方程
    ...
    def extend(self):  #粒子群状态变化函数线程
        for i in range(100):
            ...
            self.curParticle = [[
                one[0] * self.status / 100, one[1] * self.status / 100
    ...
    def __repr__(self):
        return ('color:{color}\n'
                ...
                'particle:\n{p}\n').format(color=self.color,
                                           ...
                                           cx=self.center[0],
                                           cy=self.center[1],
                                           p=str(self.oneParticle),
                                           ea=self.ellipsePara[0],
                                           eb=self.ellipsePara[1])
...
def colorChange(fire):
    ...
    f = lambda x, c: hex(int(int(x, 16) *
                             (100 - c) / 30))[2:]  #当粒子寿命到70%时，颜色开始线性衰减
    if cs > 70:
...
def appendFirework(n=1):  #递归生成烟花对象
    if n > maxFireworks or len(Fireworks) > maxFireworks:
        ...
        cl = '#{0:0>6}'.format(hex(int(random.randint(
            0, 16777215)))[2:])  # 产生一个0~16777215（0xFFFFFF）的随机数，作为随机颜色
